...
import { transformParagraphFlow } from './lib/docx-paragraph-flow.mjs';
const root = path.resolve(import.meta.dirname, '..');
const baselineCommit = '7d493f5fbd7978c47e96e26f0e1570e2a81115c7';
const runtimeVersion = '0.3.32+compat.20260923.flow';
const pinPath = path.join(root, 'patches/docx-engine-compatibility.json');
const baselinePath = path.join(root, 'test/docx-paragraph-flow/baseline.json');
...
const workspaceTemp = await mkdtemp(path.join(tmpdir(), 'docx-flow-'));
...
try {
  ...
  if (process.argv.includes('--baseline-only')) {
    ...
    for (const name of names) {
      ...
      const result = transformParagraphFlow(await readFile(path.join(dist, name), 'utf8'), 'optimized', name);
    ...
    assert.equal(assets.split("'0.3.32+compat.20260922'").length, 2, 'prior Worker cache identity');
    const resultPin = { ...pin, patchSha256, sha256: hashes, runtimeVersion, paragraphFlow: {
      baselineCommit, baselinePinSha256: sha(await readFile(baselinePath)),
      transform: 'scripts/lib/docx-paragraph-flow.mjs',
      transformSha256: sha(await readFile(path.join(root, 'scripts/lib/docx-paragraph-flow.mjs'))),
    ...
    await writeFile(assetsPath, assets.replace("'0.3.32+compat.20260922'", JSON.stringify(runtimeVersion)));
