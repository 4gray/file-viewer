...
import { transformChartAxes } from "./lib/docx-chart-axes.mjs";
const root = path.resolve(import.meta.dirname, "..");
const baselineCommit = "078ee358e2a7c777b3b74ec8c5b1e88521171c5a";
const runtimeVersion = "0.3.32+compat.20260925.axes";
const pinPath = path.join(root, "patches/docx-engine-compatibility.json");
const baselinePath = path.join(root, "test/docx-chart-axes/baseline.json");
...
const dist = path.join(
  path.dirname(require.resolve("@file-viewer/docx/package.json")),
  "dist",
);
const baseDir = path.resolve(
  process.env.DOCX_AXES_BASE_DIR ||
    path.join(root, "output/docx-chart-axes/base"),
);
const sha = (bytes) => createHash("sha256").update(bytes).digest("hex");
const names = Object.keys(baseline.sha256);
const workspaceTemp = await mkdtemp(path.join(tmpdir(), "docx-axes-"));
function git(args, cwd = workspaceTemp, expected = [0]) {
  ...
  if (r.error || !expected.includes(r.status))
    throw new Error(`git ${args[0]} failed: ${r.error || r.stderr}`);
  return r.stdout;
}
async function verify(directory, expected) {
  for (const [name, hash] of Object.entries(expected))
    assert.equal(sha(await readFile(path.join(directory, name))), hash, name);
}
try {
  ...
  if (process.argv.includes("--baseline-only")) {
    ...
    for (const name of names)
      await copyFile(path.join(dist, name), path.join(recovered, "dist", name));
    git(["apply", "--reverse", "--unsafe-paths", patchPath], recovered);
    const previousPatch = git(
      ["show", `${baselineCommit}:${patchRelative}`],
      root,
    );
    assert.equal(
      sha(previousPatch),
      baseline.patchSha256,
      "comparison Git object checksum",
    );
    ...
    assert.equal(
      pin.patchSha256,
      baseline.patchSha256,
      "this update must start from the specified baseline",
    );
    ...
    for (const dir of [original, modified])
    ...
    combined = combined
      .replaceAll("a/original/dist/", "a/dist/")
      .replaceAll("b/modified/dist/", "b/dist/");
    ...
    for (const name of names) {
      ...
      assert.deepEqual(
        await readFile(path.join(original, "dist", name)),
        result,
        name,
      );
    ...
    assert.ok(
      lock.includes(baseline.patchSha256),
      "declared pnpm patch identity",
    );
    const assetsPath = path.join(root, "packages/core/src/platform/assets.ts");
    const assets = await readFile(assetsPath, "utf8");
    assert.equal(
      assets.split(JSON.stringify(baseline.runtimeVersion)).length,
      2,
      "prior Worker cache identity",
    );
    const resultPin = {
      ...pin,
      patchSha256,
      ...
      chartAxes: {
        baselineCommit,
        baselinePinSha256: sha(await readFile(baselinePath)),
        transform: "scripts/lib/docx-chart-axes.mjs",
        transformSha256: sha(
          await readFile(path.join(root, "scripts/lib/docx-chart-axes.mjs")),
        ),
        method:
          "hash-pinned build-time transformation; no upstream source-build claim",
      },
    };
    ...
    await writeFile(
      lockPath,
      lock.replaceAll(baseline.patchSha256, patchSha256),
    );
    await writeFile(
      assetsPath,
      assets.replace(
        JSON.stringify(baseline.runtimeVersion),
        JSON.stringify(runtimeVersion),
      ),
    );
    for (const name of names)
      await copyFile(path.join(modified, "dist", name), path.join(dist, name));
    await mkdir(path.join(root, "apps/viewer-demo/public/vendor/docx"), {
      recursive: true,
    });
    await copyFile(
      path.join(dist, "docx-preview.worker.js"),
      path.join(root, "apps/viewer-demo/public/vendor/docx/docx.worker.js"),
    );
