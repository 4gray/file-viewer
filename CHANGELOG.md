...
## File Viewer v3.1.2 - 2026-09-20

- 修复 #300：EPUB 阅读器使用稳定视口判断横竖屏，避免工具栏尺寸变化引发反复重排与闪烁。已用报告者附件和浏览器回归验证。
- 修复 #304：Patch 左右对照与匹配行的布局保持紧凑；增加复现样例与回归检查，不再把有效补丁误判为格式错误。
- 新增显式启用的 BPMN 流程图预览（#297）和 XML XSD 校验、XSLT 转换视图（#305）。普通 XML 文本预览和既有 Full 包依赖保持不变；Demo 提供可体验样例。
- 同步 ZIP 安全修复、IDML 运行资源与依赖更新；保留 Vue 2.6/2.7 与既有 Full 契约。

本次发布包含 89 个 npm 目标：88 个主线包为 `3.1.2`，历史兼容别名 `msdoc-viewer` 为 `0.2.9`。DOCX 引擎保持 `0.3.32`，表格引擎保持 `0.21.6`。#227、#248、#269 尚缺报告者原始文件，不能以构造样例宣称原问题已修复。
