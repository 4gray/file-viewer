...
[`ecosystem/format-catalog.json`](ecosystem/format-catalog.json) 是唯一格式事实源：当前源码注册 273 个不重复扩展名和 46 条预览链路，其中 224 个稳定、49 个实验。下表按文件家族列出已注册扩展名。注册并不代表所有格式都能高还原：具体支持级别、已知限制和验证样例见 [格式矩阵](https://doc.file-viewer.app/guide/formats)，上线前请用业务文件验证。
